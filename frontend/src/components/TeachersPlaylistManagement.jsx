...
const VideoUploadForm = () => {
  ...
  const handleVideoChange = (e) => {
    ...
    if (file) {
      ...
      if (!['mp4', 'mov'].includes(fileExt)) {
        setMessage({ text: 'Video type not supported. Use mp4 or mov files.', type: 'error' });
        return;
      }
  ...
  const handleImageChange = (e) => {
    ...
    if (file) {
      ...
      if (!['jpg', 'jpeg', 'png'].includes(fileExt)) {
        setMessage({ text: 'Image type not supported. Use jpg, jpeg or png files.', type: 'error' });
        return;
      }
  ...
  const handleSubmit = async (e) => {
    ...
    if (!video || !image) {
      setMessage({ text: 'Please upload both video and thumbnail image', type: 'error' });
      return;
    }
  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-lg">
      <h2 className="text-2xl font-bold mb-6 text-indigo-700">Upload Educational Video</h2>
                ...
                <div className="flex flex-col items-center justify-center pt-5 pb-6">
                  {videoPreview ? (
                    ...
                    <>
                      ...
                      <p className="text-xs text-gray-500">MP4 or MOV (MAX. 1MB)</p>
                    </>
                  )}
                </div>
                <input 
